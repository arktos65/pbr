module ProductBoard
  module Resource
    class FieldFactory < ProductBoard::BaseFactory # :nodoc:
      delegate_to_target_class :map_fields, :name_to_id, :field_map
    end

    class Field < ProductBoard::Base
      ...
      def self.safer_name(description, pb_id)
        "#{safe_name(description)}_#{pb_id.split('_')[1]}"
