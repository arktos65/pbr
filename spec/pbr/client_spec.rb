...
RSpec.shared_examples 'Client Common Tests' do
  it { is_expected.to be_a ProductBoard::Client }
  ...
  it 'prepends context path to rest base_path' do
    ...
    defaults = ProductBoard::Client::DEFAULT_OPTIONS
  ...
  describe 'Resource Factories' do
    it 'gets all projects' do
      expect(ProductBoard::Resource::Project).to receive(:all).with(subject).and_return([])
      expect(subject.Project.all).to eq([])
    end

    it 'finds a single project' do
      find_result = double
      expect(ProductBoard::Resource::Project).to receive(:find).with(subject, '123').and_return(find_result)
  ...
  describe 'SSL client options' do
    context 'without certificate and key' do
      ...
      subject { ProductBoard::Client.new(options) }
...
RSpec.shared_examples 'OAuth Common Tests' do
  ...
  specify { expect(subject.request_client).to be_a ProductBoard::OauthClient }
...
describe ProductBoard::Client do
  ...
  context 'behaviour that applies to all client classes irrespective of authentication method' do
    it 'allows the overriding of some options' do
      client = ProductBoard::Client.new(consumer_key: 'foo', consumer_secret: 'bar', site: 'http://foo.com/')
      expect(client.options[:site]).to eq('http://foo.com/')
      expect(ProductBoard::Client::DEFAULT_OPTIONS[:site]).not_to eq('http://foo.com/')
    end
  end

  context 'with basic http authentication' do
    subject { ProductBoard::Client.new(username: 'foo', password: 'bar', auth_type: :basic) }
