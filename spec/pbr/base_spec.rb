...
describe ProductBoard::Base do
  class ProductBoardDelegation < SimpleDelegator # :nodoc:
  end

  class ProductBoard::Resource::Deadbeef < ProductBoard::Base # :nodoc:
  end

  class ProductBoard::Resource::HasOneExample < ProductBoard::Base # :nodoc:
    has_one :deadbeef
    has_one :muffin, class: ProductBoard::Resource::Deadbeef
    has_one :brunchmuffin, class: ProductBoard::Resource::Deadbeef,
            nested_under: 'nested'
    has_one :breakfastscone,
            class: ProductBoard::Resource::Deadbeef,
  ...
  class ProductBoard::Resource::HasManyExample < ProductBoard::Base # :nodoc:
    has_many :deadbeefs
    has_many :brunchmuffins, class: ProductBoard::Resource::Deadbeef,
             nested_under: 'nested'
  ...
  subject { ProductBoard::Resource::Deadbeef.new(client, attrs: attrs) }

  let(:decorated) { ProductBoardDelegation.new(subject) }
  ...
  it 'returns all the deadbeefs' do
    ...
    expect(ProductBoard::Resource::Deadbeef).to receive(:collection_path).and_return('/jira/rest/api/2/deadbeef')
    deadbeefs = ProductBoard::Resource::Deadbeef.all(client)
    expect(deadbeefs.length).to eq(1)
    first = deadbeefs.first
    expect(first.class).to eq(ProductBoard::Resource::Deadbeef)
  ...
  it 'finds a deadbeef by id' do
    ...
    expect(ProductBoard::Resource::Deadbeef).to receive(:collection_path).and_return('/jira/rest/api/2/deadbeef')
    deadbeef = ProductBoard::Resource::Deadbeef.find(client, '98765')
  ...
  it 'finds a deadbeef containing changelog by id' do
    ...
    deadbeef = ProductBoard::Resource::Deadbeef.find(client, '98765', expand: 'changelog')
    expect(deadbeef.client).to eq(client)
    expect(deadbeef.attrs['self']).to eq('http://deadbeef/')
    expect(deadbeef.attrs['id']).to eq('98765')
    expect(deadbeef.expanded?).to be_truthy
  ...
  it 'builds a deadbeef' do
    deadbeef = ProductBoard::Resource::Deadbeef.build(client, 'id' => '98765')
  ...
  describe 'fetch' do
    subject { ProductBoard::Resource::Deadbeef.new(client, attrs: { 'id' => '98765' }) }
  ...
  describe 'save' do
    ...
    subject { ProductBoard::Resource::Deadbeef.new(client) }
    ...
    it 'returns false when an invalid field is set' do # The ProductBoard REST API apparently ignores fields that you aren't allowed to set manually
      response = instance_double('Response', body: '{"errorMessages":["blah"]}', status: 400)
      allow(subject).to receive(:new_record?) { false }
      expect(client).to receive(:put).with('/foo/bar', '{"invalid_field":"foobar"}').and_raise(ProductBoard::HTTPError.new(response))
    ...
    it 'returns false with exception details when non json response body (unauthorized)' do # Unauthorized requests return a non-json body. This makes sure we can handle non-json bodies on HTTPError
      ...
      expect(client).to receive(:post).with('/foo/bar', '{"foo":"bar"}').and_raise(ProductBoard::HTTPError.new(response))
  ...
  describe 'save!' do
    ...
    it 'throws an exception when an invalid field is set' do
      ...
      allow(subject).to receive(:new_record?) { false }
      expect(client).to receive(:put).with('/foo/bar', '{"invalid_field":"foobar"}').and_raise(ProductBoard::HTTPError.new(response))
      expect(-> { subject.save!('invalid_field' => 'foobar') }).to raise_error(ProductBoard::HTTPError)
    end
  end
  ...
  describe 'url' do
    ...
    it 'has a class method for the collection path' do
      expect(ProductBoard::Resource::Deadbeef.collection_path(client)).to eq('/foo/bar/deadbeef')
      # Should accept an optional prefix (flum in this case)
      expect(ProductBoard::Resource::Deadbeef.collection_path(client, '/flum/')).to eq('/foo/bar/flum/deadbeef')
    end

    it 'has a class method for the singular path' do
      expect(ProductBoard::Resource::Deadbeef.singular_path(client, 'abc123')).to eq('/foo/bar/deadbeef/abc123')
      # Should accept an optional prefix (flum in this case)
      expect(ProductBoard::Resource::Deadbeef.singular_path(client, 'abc123', '/flum/')).to eq('/foo/bar/flum/deadbeef/abc123')
    end
  end
  ...
  it 'returns the formatted attrs from to_s' do
    ...
    expect(subject.to_s).to match(/#<ProductBoard::Resource::Deadbeef:\d+ @attrs=#{Regexp.quote(attrs.inspect)}>/)
  end
  ...
  describe 'extract attrs from response' do
    subject { ProductBoard::Resource::Deadbeef.new(client, attrs: {}) }
  ...
  describe 'nesting' do
    it 'defaults collection_attributes_are_nested to false' do
      expect(ProductBoard::Resource::Deadbeef.collection_attributes_are_nested).to be_falsey
    end

    it 'allows collection_attributes_are_nested to be set' do
      ProductBoard::Resource::Deadbeef.nested_collections true
      expect(ProductBoard::Resource::Deadbeef.collection_attributes_are_nested).to be_truthy
    end
  end

  describe 'has_many' do
    subject { ProductBoard::Resource::HasManyExample.new(client, attrs: { 'deadbeefs' => [{ 'id' => '123' }] }) }

    it 'returns a collection of instances for has_many relationships' do
      expect(subject.deadbeefs.class).to eq(ProductBoard::HasManyProxy)
      expect(subject.deadbeefs.length).to eq(1)
      subject.deadbeefs.each do |deadbeef|
        expect(deadbeef.class).to eq(ProductBoard::Resource::Deadbeef)
      end
    end

    it 'returns an empty collection for empty has_many relationships' do
      subject = ProductBoard::Resource::HasManyExample.new(client)
      expect(subject.deadbeefs.length).to eq(0)
    end

    it 'allows the has_many attributes to be nested inside another attribute' do
      subject = ProductBoard::Resource::HasManyExample.new(client, attrs: { 'nested' => { 'brunchmuffins' => [{ 'id' => '123' }, { 'id' => '456' }] } })
      expect(subject.brunchmuffins.length).to eq(2)
      subject.brunchmuffins.each do |brunchmuffin|
        expect(brunchmuffin.class).to eq(ProductBoard::Resource::Deadbeef)
      end
    end

    it 'allows it to be deeply nested' do
      subject = ProductBoard::Resource::HasManyExample.new(client, attrs: { 'nested' => {
      ...
      subject.breakfastscones.each do |breakfastscone|
        expect(breakfastscone.class).to eq(ProductBoard::Resource::Deadbeef)
      end
    end

    it 'short circuits missing deeply nested attrs' do
      subject = ProductBoard::Resource::HasManyExample.new(client, attrs: {
        'nested' => {}
      })
    ...
    it 'allows the attribute key to be specified' do
      subject = ProductBoard::Resource::HasManyExample.new(client, attrs: { 'irregularlyNamedThings' => [{ 'id' => '123' }, { 'id' => '456' }] })
      expect(subject.irregularly_named_things.length).to eq(2)
      subject.irregularly_named_things.each do |thing|
        expect(thing.class).to eq(ProductBoard::Resource::Deadbeef)
      end
    end
  ...
  describe 'has_one' do
    subject { ProductBoard::Resource::HasOneExample.new(client, attrs: { 'deadbeef' => { 'id' => '123' } }) }

    it 'returns an instance for a has one relationship' do
      expect(subject.deadbeef.class).to eq(ProductBoard::Resource::Deadbeef)
      expect(subject.deadbeef.id).to eq('123')
    end

    it 'returns nil when resource attribute is nonexistent' do
      subject = ProductBoard::Resource::HasOneExample.new(client)
      expect(subject.deadbeef).to be_nil
    end

    it 'returns an instance with a different class name to the attribute name' do
      subject = ProductBoard::Resource::HasOneExample.new(client, attrs: { 'muffin' => { 'id' => '123' } })
      expect(subject.muffin.class).to eq(ProductBoard::Resource::Deadbeef)
      expect(subject.muffin.id).to eq('123')
    end

    it 'allows the has_one attributes to be nested inside another attribute' do
      subject = ProductBoard::Resource::HasOneExample.new(client, attrs: { 'nested' => { 'brunchmuffin' => { 'id' => '123' } } })
      expect(subject.brunchmuffin.class).to eq(ProductBoard::Resource::Deadbeef)
      expect(subject.brunchmuffin.id).to eq('123')
    end

    it 'allows it to be deeply nested' do
      subject = ProductBoard::Resource::HasOneExample.new(client, attrs: { 'nested' => {
        'breakfastscone' => { 'breakfastscone' => { 'id' => '123' } }
      } })
      expect(subject.breakfastscone.class).to eq(ProductBoard::Resource::Deadbeef)
      expect(subject.breakfastscone.id).to eq('123')
    end

    it 'allows the attribute key to be specified' do
      subject = ProductBoard::Resource::HasOneExample.new(client, attrs: { 'irregularlyNamedThing' => { 'id' => '123' } })
      expect(subject.irregularly_named_thing.class).to eq(ProductBoard::Resource::Deadbeef)
      expect(subject.irregularly_named_thing.id).to eq('123')
    end
  end

  describe 'belongs_to' do
    class ProductBoard::Resource::BelongsToExample < ProductBoard::Base
      belongs_to :deadbeef
    end

    let(:deadbeef) { ProductBoard::Resource::Deadbeef.new(client, attrs: { 'id' => '999' }) }

    subject { ProductBoard::Resource::BelongsToExample.new(client, attrs: { 'id' => '123' }, deadbeef: deadbeef) }
    ...
    it 'raises an exception when initialized without a belongs_to instance' do
      expect(lambda {
        ProductBoard::Resource::BelongsToExample.new(client, attrs: { 'id' => '123' })
    ...
    it 'can be initialized with an instance or a key value' do
      allow(client).to receive(:options) { { rest_base_path: '/foo' } }
      subject = ProductBoard::Resource::BelongsToExample.new(client, attrs: { 'id' => '123' }, deadbeef_id: '987')
